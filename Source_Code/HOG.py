...
def makeImagesGrayscale(imageDir, verbose: bool):
	if verbose:
		print("<NoClass>: Running function -> 'makeImagesGrayscale' ...")
	grayImages = []
...
def rescale_image(imageDir, verbose: bool, res_x=128, res_y=64, ):
	if verbose:
		print("<NoClass>: Running function -> 'rescale_image' ...")
	rscImages = []
	for image in range(0, len(imageDir)):
...
if __name__ == "__main__":
	...
	rscImgLand = rescale_image(hsvImgLand, 256, 256, verbose=True)
	rscImgPort = rescale_image(hsvImgPort, 256, 256, verbose=True)

	hogFeaturesLand = HOG(rscImgLand, verbose=True)
	hogFeaturesPort = HOG(rscImgPort, verbose=True)
